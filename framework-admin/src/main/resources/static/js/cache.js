...
layui.use(['form','jquery',"layer"],function() {
    ...
    //公告层
    function showNotice(){
    	$.post(main+"/admin/system/notice/getNotice", function(data) {
		 	if(null!=data.data){
		        ...
		        layer.open({
		            ...
		            btn: ['我知道了'],
		            moveType: 1,
		            content: '<div style="padding:15px 20px; text-align:justify; line-height: 22px; text-indent:2em;border-bottom:1px solid #e2e2e2;"><p class="layui-red">'+content+'</span></p></div>',
    ...
    $("body").on("click","#unlock",function(){
        if($(this).siblings(".admin-header-lock-input").val() == ''){
        	...
        	checkPassword($(this).siblings(".admin-header-lock-input").val());
...
//            }
    ...
    function checkPassword(password){
    	  $.post(main+"/admin/system/user/checkpwd",{password:password},function(result){
    		  if(result.code==true){
	                window.sessionStorage.setItem("lockcms",false);
	                $(this).siblings(".admin-header-lock-input").val('');
	                layer.closeAll("page");
