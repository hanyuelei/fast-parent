...
layui.define(["element","jquery",'domain'],function(exports){
	var element = layui.element,
		$ = layui.$,
		layId,
		domain=layui.cache.main,
		Tab = function(){
			this.tabConfig = {
            ...
            if(data[i].children != undefined && data[i].children.length > 0){
                ...
                for(var j=0;j<data[i].children.length;j++){
                    if(data[i].children[j].target == "_blank"){
                        ulHtml += '<dd><a data-url="'+main+data[i].children[j].href+'" target="'+data[i].children[j].target+'">';
                    }else{
                        ulHtml += '<dd><a data-url="'+main+data[i].children[j].href+'">';
                    }
                ...
                if(data[i].target == "_blank"){
                    ulHtml += '<a data-url="'+main+data[i].href+'" target="'+data[i].target+'">';
                }else{
                    ulHtml += '<a data-url="'+main+data[i].href+'">';
                }
                if(data[i].icon != undefined && data[i].icon != ''){
                    if(data[i].icon.indexOf("icon-") != -1){
                        ulHtml += '<i class="seraph '+data[i].icon+'" data-icon="'+data[i].icon+'"></i>';
